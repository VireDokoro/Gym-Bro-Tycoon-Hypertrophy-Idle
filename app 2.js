...
function refreshAll() {
  try { updateHUD(); }           catch(e) { console.warn('HUD', e); }
  try { updateCharacterStage(); } catch(e) { console.warn('Stage', e); }
  try { buildShopItems(); }      catch(e) { console.warn('Shop', e); }
  try { updatePrestigePanel(); } catch(e) { console.warn('Prestige', e); }
  try { renderMissions(); }      catch(e) { console.warn('Missions', e); }
  try { renderAchievements(); }  catch(e) { console.warn('Ach', e); }
  try { renderStats(); }         catch(e) { console.warn('Stats', e); }
  try { updateTapLabel(); }      catch(e) { console.warn('TapLabel', e); }
}
...
function runSplash(onDone) {
  ...
  function step() {
    ...
    if (progress >= 100) {
      ...
      // Small pause so the user sees the full bar, then fade out
  ...
  function finish() {
    DOM.splash.style.transition = 'opacity 0.4s ease';
  ...
  // Start after a tiny delay so the browser has painted the splash first
  setTimeout(step, 80);
}
...
function boot() {
  ...
  // Run splash first — bind events and start loop only after app is visible
  runSplash(() => {
    bindEvents();
    refreshAll();
...
// Wait for DOM — use both DOMContentLoaded and a safety fallback
function safeBoot() {
  // Verify critical elements exist before booting
